...
mvn package
```
Maven should download the required libraries, compile the sources and place compiled code to the target folder.

## Installation
### Copy files
Your vmbix folder should look like this:
```
vmbix/etc/init.d/vmbixd                                # init script, will start vmbixd as daemon
vmbix/etc/vmbix/vmbix.conf                             # VmBix config file
vmbix/etc/vmbix/logback.xml                            # logging config file
vmbix/usr/local/sbin/vmbix                             # sbin script to run the tool
vmbix/usr/local/sbin/vmbixd                            # like vmbix but will start in background
vmbix/target/vmbix-[version]-jar-with-dependencies.jar # jar file with included dependencies
vmbix/target/vmbix-[version].jar                       # jar file without dependencies
vmbix/zabbix_templates/new/template_vmbix_vcenter.xml  # vCenter and datastore zabbix template
vmbix/zabbix_templates/new/template_vmbix_esx.xml      # Host zabbix template 
...
## Cache info
Versions 1.2.3+ need config file with cache settings.
